...
# Title: Creating figures and tables of ABCO (white fir) model results
# for publication
...
# datasets used to fit those models (ie. ABCO_data_yr1_full and ABCO_
# data_yr2), all of which should be stored in the Environment pane after
# running the script "03_model_fitting_ABCO.R". Additionally, 3) an ordered
...
# displaying predicted cone counts for small, medium, and big ABCOs
...
# ABCOs (only) in sample years 1 and 2 on the same axes; all figures are
...
# medium, and big ABCOs in each sample year; percent change tables
...
# and big ABCOs in each sample year; contrasts summary tables are 
...
# median field site for yr1 ABCO...

# double checking ABCO_myr1's model summary table to verify the median
# fecundity site for yr1...
summary(ABCO_myr1) # CREEK is the median fecundity site

# running Function F to generate a predictions data.frame...
ABCO_myr1_pred_df <- F_generate_predictions_df(model = ABCO_myr1,
                                               sppyr_data = ABCO_data_yr1_full,
                                               median_site = "CREEK")
...
# distinct DBH curve...
ABCO_myr1_thresholds <- ABCO_myr1_pred_df %>% 
...
ABCO_myr1_pred_df_thresholds <- H_predict_cones_at_thresholds(
  model = ABCO_myr1,
  median_site = "CREEK",
  threshold_table = ABCO_myr1_thresholds)


# plotting ABCO yr1 base model predictions; this plot includes points  
...
# range of raw data on CrownDamage) for yr1 ABCO...
ABCO_myr1_plot <- ABCO_myr1_pred_df %>% 
  ggplot(aes(x = CrownDamage_percentvolume_unscaled,
             y = fit_backtransformed,
             color = DBH_cm_unscaled)) +
  geom_line(linewidth = 1) +
  scale_color_brewer(palette = "YlGnBu",
                     labels = c("10th percentile (47.0)",
                                "50th percentile (65.3)",
                                "90th percentile (93.3)")) +
...
# for yr1 ABCO...
ABCO_myr1_lookup_table <- I_create_lookup_table(
  sppyr_data = ABCO_data_yr1_full,
  median_site = "CREEK",
  threshold_table = ABCO_myr1_thresholds)

# running Function J to pivot this lookup table wider and add a column
# that holds the attributes of ABCO_myr1...
ABCO_myr1_lookup_table_wide <- J_pivot_lookup_table_wider(
  model = ABCO_myr1,
  lookup_table = ABCO_myr1_lookup_table)

# running Function K to create the ABCO_myr1 percent change table...
ABCO_myr1_percentchange_table <- K_create_percentchange_table(
  model = ABCO_myr1,
  lookup_table = ABCO_myr1_lookup_table,
  threshold_table = ABCO_myr1_thresholds)

# running Functions L2 and L3 to create and then format the ABCO_myr1
# contrasts summary table...
ABCO_myr1_contrasts_summary_table <- L2_create_contrasts_summary_table(
  lookup_table_wide = ABCO_myr1_lookup_table_wide)

ABCO_myr1_contrasts_summary_table_wide <- L3_format_contrasts_summary_table(
  contrasts_summary_table = ABCO_myr1_contrasts_summary_table,
  threshold_table = ABCO_myr1_thresholds)
...
# three distinct tree sizes (the 10th, 50th, and 90th percentile-
# sized trees), along the full spectrum of CrownDamage, and at the
# median field site for yr2 ABCO...

# double checking ABCO_myr2's model summary table to verify the median
# fecundity site for yr2...
summary(ABCO_myr2) # CREEK is still the median fecundity site

# running Function F to generate a predictions data.frame...
ABCO_myr2_pred_df <- F_generate_predictions_df(model = ABCO_myr2,
                                               sppyr_data = ABCO_data_yr2,
                                               median_site = "CREEK")

# running Function G2 to identify the threshold value (ie. the CrownDamage
# value at which predicted cone counts decline most quickly) along each
# distinct DBH curve...
ABCO_myr2_thresholds <- ABCO_myr2_pred_df %>% 
  group_by(DBH_cm_scaled) %>% 
  summarize(threshold_val = G2_identify_thresholds_minderiv2(
    x_values = CrownDamage_percentvolume_scaled,
    curve_values = fit,
...
# running Function H to predict total conservative cone counts at these
# identified threshold values...
ABCO_myr2_pred_df_thresholds <- H_predict_cones_at_thresholds(
  model = ABCO_myr2,
  median_site = "CREEK",
  threshold_table = ABCO_myr2_thresholds)


# plotting ABCO yr2 base model predictions; this plot includes points  
# that represent the predicted threshold values for each DBH curve, and 
# a rug plot that displays field-collected CrownDamage values (ie. the 
# range of raw data on CrownDamage) for yr2 ABCO...
ABCO_myr2_plot <- ABCO_myr2_pred_df %>% 
  ggplot(aes(x = CrownDamage_percentvolume_unscaled,
             y = fit_backtransformed,
             color = DBH_cm_unscaled)) +
  geom_line(linewidth = 1) +
...
# the threshold value, for each curve), and the median fecundity site 
# for yr2 ABCO...
ABCO_myr2_lookup_table <- I_create_lookup_table(
  sppyr_data = ABCO_data_yr2,
  median_site = "CREEK",
  threshold_table = ABCO_myr2_thresholds)

# running Function J to pivot this lookup table wider and add a column
# that holds the attributes of ABCO_myr2...
ABCO_myr2_lookup_table_wide <- J_pivot_lookup_table_wider(
  model = ABCO_myr2,
  lookup_table = ABCO_myr2_lookup_table)

# running Function K to create the ABCO_myr2 percent change table...
ABCO_myr2_percentchange_table <- K_create_percentchange_table(
  model = ABCO_myr2,
  lookup_table = ABCO_myr2_lookup_table,
  threshold_table = ABCO_myr2_thresholds)

# rearranging the columns of the ABCO_myr2 percent change table,
# to reflect the fact that the threshold values for all three
# DBH curves are <50%, and to facilitate table interpretation...
ABCO_myr2_percentchange_table <- ABCO_myr2_percentchange_table %>% 
  select(c(1:3, 5, 4, 6, 8, 7, 10, 9, 11))

# running Functions L2 and L3 to create and then format the ABCO_myr2
# contrasts summary table...
ABCO_myr2_contrasts_summary_table <- L2_create_contrasts_summary_table(
  lookup_table_wide = ABCO_myr2_lookup_table_wide)

ABCO_myr2_contrasts_summary_table_wide <- L3_format_contrasts_summary_table(
  contrasts_summary_table = ABCO_myr2_contrasts_summary_table,
  threshold_table = ABCO_myr2_thresholds)

# again, rearranging the columns of the ABCO_myr2 contrasts table, 
# to reflect the fact that threshold values are <50%, and to facilitate
# table interpretation...
ABCO_myr2_contrasts_summary_table_wide <-
  ABCO_myr2_contrasts_summary_table_wide %>% 
...
## ABCO in sample years 1 and 2 on the same set of axes...
...
ABCO_myr1_pred_df_mediantree <- ABCO_myr1_pred_df %>% 
  filter(DBH_cm_scaled == median(DBH_cm_scaled)) %>% 
  mutate(SampleEffort_yr = as.factor("yr1"))

ABCO_myr2_pred_df_mediantree <- ABCO_myr2_pred_df %>% 
  filter(DBH_cm_scaled == median(DBH_cm_scaled)) %>% 
  mutate(SampleEffort_yr = as.factor("yr2"))

# joining these median tree predictions data.frames together...
ABCO_mediantree_pred_df_BOTHYRS <- bind_rows(ABCO_myr1_pred_df_mediantree,
                                             ABCO_myr2_pred_df_mediantree)

# repeating these same steps for the yr1 and yr2 threshold predictions
# data.frames...
ABCO_myr1_pred_thresholds_mediantree <- ABCO_myr1_pred_df_thresholds %>% 
  filter(DBH_cm_scaled == median(DBH_cm_scaled)) %>% 
  mutate(SampleEffort_yr = as.factor("yr1"))

ABCO_myr2_pred_thresholds_mediantree <- ABCO_myr2_pred_df_thresholds %>% 
  filter(DBH_cm_scaled == median(DBH_cm_scaled)) %>% 
  mutate(SampleEffort_yr = as.factor("yr2"))

ABCO_mediantree_pred_thresholds_BOTHYRS <- bind_rows(
  ABCO_myr1_pred_thresholds_mediantree,
  ABCO_myr2_pred_thresholds_mediantree)
...
ABCO_mediantree_plot <- ABCO_mediantree_pred_df_BOTHYRS %>% 
  ggplot(aes(x = CrownDamage_percentvolume_unscaled,
             y = fit_backtransformed,
...
ABCO_mediantree_plot

# adding a rug plot to the ABCO_mediantree_plot, to test legibility...
ABCO_mediantree_plot_wrug <- ABCO_mediantree_plot + 
  geom_rug(data = ABCO_data_yr1_full,
...
ABCO_mediantree_plot_wrug

# adding a stacked, color-coded rug plot to the ABCO_mediantree_plot, to 
# try to increase legibility of rug...
ABCO_mediantree_plot_wrug_stacked <- ABCO_mediantree_plot + 
...
ABCO_mediantree_plot_wrug_stacked

# adding a grey-scale rug plot to the ABCO_mediantree_plot, where 
...
ABCO_mediantree_plot_wrug_greyscale <- ABCO_mediantree_plot + 
  geom_rug(data = subset(ABCO_data_yr1_full, TreeStatus_yr2 != "A"),
           aes(x = CrownDamage_percentvolume,
               y = 5),
           color = "grey",
           sides = "b",
...
ABCO_mediantree_plot_wrug_greyscale



## exporting the final results figures, percent change tables, and
## contrasts summary tables for ABCO yr1 and yr2...
